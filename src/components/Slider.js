...
class Slider extends Component {
    ...
    componentDidMount() {
        this.interval = setInterval(() => this.intervalBetweenSliders(), 3000)
    }
    ...
    intervalBetweenSliders = () => {
        if (this.state.autoplay === true) {
            ...
            this.setState({
                active: num === this.state.slides.length -1 ? 0 : num + 1
            })
        }
    };
    ...
    nextOne = () => {
        let num = this.state.active;
        this.setState({
            active: num < this.state.slides.length - 1 ? num + 1 :  0
        });
        this.toggleAutoPlay();
    };
    ...
    prevOne = () => {
        let num = this.state.active;
        this.setState({
            active: num > 0 ? num - 1 : this.state.slides.length - 1
        });
        this.toggleAutoPlay();
    };
